...
export const IntelligenceCountryParamSchema = z.object({
...
export const MissionCompareQuerySchema = z.object({
  aircraftA: z.string().trim().min(1, 'First aircraft ID or name is required'),
  aircraftB: z.string().trim().min(1, 'Second aircraft ID or name is required'),
  mission: z
    .enum([
...
export const SitrepQuerySchema = z.object({
...
export const CountryInventoryParamsSchema = z.object({
  countryCode: z.string().trim().min(2, 'Country code must be at least 2 characters'),
});

export const NavalVesselsQuerySchema = z.object({
  country: z.string().optional(),
...
export const GroundVehiclesQuerySchema = z.object({
  country: z.string().optional(),
