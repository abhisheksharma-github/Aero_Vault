...
export default function ComparisonModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto bg-av-navy/90 backdrop-blur-2xl animate-fade-in">
      <div 
      ...
      >
        ...
        {/* Header Ribbon */}
        <div className="p-6 bg-gradient-to-r from-av-blue via-av-blue/70 to-av-navy border-b border-av-steel/30 flex-shrink-0 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="p-2.5 rounded-2xl bg-av-sky/20 border border-av-sky/40 text-av-sky shadow-glow-cyan">
            ...
            <div>
              ...
              <p className="text-xs font-mono text-av-mist">
                Head-to-head deterministic airframe combat analysis & simulated mission showdown
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="p-2.5 rounded-2xl bg-av-navy/80 text-av-mist hover:text-white hover:bg-rose-500/20 hover:border-rose-500/50 border border-av-steel/40 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto space-y-8 flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
            ...
            {/* Aircraft A Card */}
            <div className="p-5 rounded-3xl glass-panel border border-cyan-500/40 space-y-4 bg-gradient-to-b from-cyan-950/30 to-av-dark hud-corner-box">
              <div className="flex items-center justify-between gap-3">
                <select
                  ...
                  className="bg-av-navy/90 border border-cyan-500/40 text-white font-display font-bold text-base sm:text-lg rounded-xl px-3 py-1.5 focus:outline-none focus:border-av-sky w-full font-sans cursor-pointer"
                >
                  {allAircraft.map((a) => (
                ...
                <span className="px-3 py-1 rounded-xl bg-cyan-500/20 text-cyan-300 border border-cyan-400/40 text-xs font-mono font-black">
                  {tvrA.toFixed(1)} TVR
                </span>
              </div>

              <div className="h-40 rounded-2xl overflow-hidden border border-av-steel/30 bg-av-navy">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                <div className="p-2 rounded-xl bg-av-navy/80 border border-av-steel/20">
                  ...
                  <span className="text-white font-bold">{aircraftA.topSpeedMach ? `Mach ${aircraftA.topSpeedMach}` : 'Mach 2.0'}</span>
                </div>
                <div className="p-2 rounded-xl bg-av-navy/80 border border-av-steel/20">
                  <span className="text-av-mist block text-[10px]">Radar Reach:</span>
                  <span className="text-cyan-300 font-bold">{aircraftA.radarRangeAirKm ? `${aircraftA.radarRangeAirKm} km` : '200+ km'}</span>
                </div>
                <div className="p-2 rounded-xl bg-av-navy/80 border border-av-steel/20">
                  <span className="text-av-mist block text-[10px]">Stealth:</span>
                  <span className="text-purple-300 font-bold">{aircraftA.stealthLevel?.replace('_', ' ') || 'REDUCED'}</span>
                </div>
                <div className="p-2 rounded-xl bg-av-navy/80 border border-av-steel/20">
                  <span className="text-av-mist block text-[10px]">Combat Radius:</span>
                  <span className="text-emerald-400 font-bold">{aircraftA.combatRangeKm ? `${aircraftA.combatRangeKm} km` : '1,500 km'}</span>
                </div>
              </div>
            </div>

            {/* Aircraft B Card */}
            <div className="p-5 rounded-3xl glass-panel border border-emerald-500/40 space-y-4 bg-gradient-to-b from-emerald-950/30 to-av-dark hud-corner-box">
              <div className="flex items-center justify-between gap-3">
                <select
                  ...
                  className="bg-av-navy/90 border border-emerald-500/40 text-white font-display font-bold text-base sm:text-lg rounded-xl px-3 py-1.5 focus:outline-none focus:border-emerald-400 w-full font-sans cursor-pointer"
                >
                  {allAircraft.map((a) => (
                    <option key={a.name} value={a.name} className="bg-av-dark text-white font-sans">
                      {a.name} ({a.country})
                ...
                <span className="px-3 py-1 rounded-xl bg-emerald-500/20 text-emerald-300 border border-emerald-400/40 text-xs font-mono font-black">
                  {tvrB.toFixed(1)} TVR
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                <div className="p-2 rounded-xl bg-av-navy/80 border border-av-steel/20">
                  <span className="text-av-mist block text-[10px]">Speed:</span>
                  <span className="text-white font-bold">{secondAircraft.topSpeedMach ? `Mach ${secondAircraft.topSpeedMach}` : 'Mach 2.0'}</span>
                </div>
                <div className="p-2 rounded-xl bg-av-navy/80 border border-av-steel/20">
                  <span className="text-av-mist block text-[10px]">Radar Reach:</span>
                  <span className="text-cyan-300 font-bold">{secondAircraft.radarRangeAirKm ? `${secondAircraft.radarRangeAirKm} km` : '200+ km'}</span>
                </div>
                <div className="p-2 rounded-xl bg-av-navy/80 border border-av-steel/20">
                  <span className="text-av-mist block text-[10px]">Stealth:</span>
                  <span className="text-purple-300 font-bold">{secondAircraft.stealthLevel?.replace('_', ' ') || 'REDUCED'}</span>
                </div>
                <div className="p-2 rounded-xl bg-av-navy/80 border border-av-steel/20">
                  <span className="text-av-mist block text-[10px]">Combat Radius:</span>
                  <span className="text-emerald-400 font-bold">{secondAircraft.combatRangeKm ? `${secondAircraft.combatRangeKm} km` : '1,500 km'}</span>
                </div>
              </div>
            </div>
          </div>
